    ...
    try {
      String uriGetConnectorsV2 =
          URI_GET_ALL_CONNECTORS_V2
              + "?host=" + kafkaConnectHost
              + "&protocol=" + protocol
              + "&cluster=" + clusterIdentification
              + "&includeStatus=" + getConnectorsStatuses;
      String uriGetConnectorsFull = clusterConnUrl + uriGetConnectorsV2;

      log.info("Attempting v2 API: {}", uriGetConnectorsFull);
      ...
      try {
        ...
        log.error("Error from getAllKafkaConnectors v1 fallback", ex);
        throw new KlawException(CLUSTER_API_ERR_115);
      }
    }
  }
